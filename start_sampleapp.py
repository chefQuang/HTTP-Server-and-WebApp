...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS channels (name TEXT PRIMARY KEY, created_by TEXT)''')

    #c.execute("INSERT OR IGNORE INTO channels (name, created_by) VALUES ('General', 'System')")
...
def login(headers, body):
    ...
    print("[SampleApp] Login attempt: user={}, pass={}".format(username, password))
...
def create_channel(headers, body):
    user = get_user_from_cookie(headers)
    if not user: return json.dumps({"error":"Unauthorized"}), 401, {}
    
    data = parse_from_data(body)
    channel_name = data.get('name')
    if not channel_name: return json.dumps({"error":"Missing name"}), 400, {}

    try:
        conn = get_db_connection()
        conn.execute("INSERT INTO channels (name, created_by) VALUES (?,?)", (channel_name, user))
        conn.commit()
        conn.close()
        return json.dumps({"status":"created", "name": channel_name}), 200, {'Content-Type':'application/json'}
    except sqlite3.IntegrityError:
        return json.dumps({"error":"Channel exists"}), 409, {'Content-Type':'application/json'}
...
def list_channels(headers, body):
    ...
    conn.close()
    channels = [{"name": r['name'], "creator": r['created_by']} for r in rows]
    return json.dumps(channels), 200, {'Content-Type':'application/json'}
