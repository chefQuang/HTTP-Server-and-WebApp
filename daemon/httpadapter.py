...
class HttpAdapter:
    ...
    def handle_client(self, conn, addr, routes):
        ...
        if req.hook:
            ...
            #
            #Gọi hàm hook và truyền headers và body để nó xử lý
            try:
                result = req.hook(headers=req.headers, body=req.body)
                if isinstance(result, tuple) and len(result) == 3:
                    response_data, status_code, extra_headers = result
                else:
                ...
                #Gán nội dung vào response nè
                resp._content = content
                resp.status_code = status_code
                resp.reason = "OK" if status_code == 200 else "Unauthorized"

                #Merge headers để nhét cookie vào nè
                if extra_headers:
                    resp.headers.update(extra_headers)

                #Build header và gửi response
                header_bytes = resp.build_response_header(req)
                conn.sendall(header_bytes + content)

            except Exception as e:
                print("[HttpAdapter] Error in hook: {}".format(e))
                resp.status_code = 500
                resp._content = b"500 Internal Server Error"
