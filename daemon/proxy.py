...
def resolve_routing_policy(hostname, routes, path=""):
    ...
    proxy_host = '127.0.0.1' #IP mặc định của backend
    proxy_port = '9000'
        ...
        #if len(proxy_map) > 0:
            #proxy_host, proxy_host = proxy_map[0].split(":", 1)
    ...
    app_keywords = ['/login', '/chat', 'user', '/register', '/get-list', '/submit-info', '/logout', '/get-my-info']
    if any(keyword in path for keyword in app_keywords):
        print("[Routing] Path '{}' detected -> WebApp (8000)".format(path))
        return '127.0.0.1', 8000  #Nếu test trên VM thì phải đổi thành ip của VM đó
